...
groupSize=len(fits)
#switch to soc for group rounds
...
#uncomment for non nLL version - non nLL have r² for all models, but nLL for only one, nLL is vice versa
...
#parfits has all models + parameters
parfits = pd.merge(parfits,pars)
parfits.to_csv("./Data/fit+pars_e2_ind_nLL.csv",index=False)

#fit only has parameters for winning model
...
for i in range(len(fit)):
    ...
    if highest==0:
        ...
        fit.loc[i,"lambda":"par"]=pars.loc[(pars["agent"]==fit.loc[i,"agent"]) & (pars["group"]==fit.loc[i,"group"]) &
                  (pars["model"]==fit.loc[i,"model"]),["lambda","beta","tau","par"]].T.to_numpy()[:,0]
fit.to_csv("./Data/model_fits_e2_ind_nLL.csv",index=False)


# compute and save protected exceedance probability
from groupBMC.groupBMC import GroupBMC

fit = pd.read_csv("./Data/model_fits_e2_ind_nLL.csv")
...
result.to_csv("./Data/pxp_e2_ind.csv",index=False)
