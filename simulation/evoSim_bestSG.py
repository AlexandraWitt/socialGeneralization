...
#we only want SG, so type mutations are out
typeMutation = 0
...
for gen in range(1,generations):
    ...
    for i in range(popSize):
        #bounds based on 40-60% quantiles of parameter values
        if newPop[i][0]['lambda']<0.98 or newPop[i][0]['lambda']>1.3:
            newPop[i][0]['lambda'] = np.random.uniform(0.98,1.3)
        if newPop[i][0]['beta']<0.17 or newPop[i][0]['beta']<0.35:
            newPop[i][0]['beta'] = np.random.uniform(0.17,0.35)
        if newPop[i][0]['tau'] <0.045 or newPop[i][0]['tau']>0.063:
            newPop[i][0]['tau'] = np.random.uniform(0.045,0.063)
        ...
        if newPop[i][0]['eps_soc']<0 or newPop[i][0]['eps_soc']>19:
            newPop[i][0]['eps_soc'] = np.random.uniform(0,19)
...
path = "./Data/evoSims/evoSim_SGbest/"
if not os.path.exists(path):
    os.makedirs(path)
filename = path+name+"_SGbest_"+str(mix) 
np.save(filename,outputList)

filename = path+name+"_SGbest_"+str(mix)+"_scores"
